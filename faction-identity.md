...
**Open before any of this is locked:** every exact number beyond what's stated above (HAMR Rifle's and HAMR Carbine's own base profiles, all three melee-swap weapons' AP/Damage, `Crushing`'s and the Gravity Pistol's full profile, Oathreapers' grenade name and mechanism, `Deployment (Infiltrate)`'s exact distance, `Optics`'s pricing tier); Heavies/Terminators still has no package writeup and no name locked (Errant and Oathwarden both now have full writeups, above); Oathmaster has a name and nothing else; and the roster still needs whatever units answer SUPPORT and ARMOR for this faction beyond the LINE/COMMAND/RECON/SHOCK work done here.
