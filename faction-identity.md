...
The third and heaviest weight class, and the only one built as a **two-vehicle twin pair from a single defining rule** rather than a broader family: **Wolverine always has a turreted main gun, Badger never does.** Everything else about the two follows from that one split. Regiments-specific, same as the rest of the roster. **Stat line not yet set** — the naming pass had explicitly excluded Wolverine from the shared-profile system entirely ("apex, solitary"), so unlike Light Chassis there's no inherited baseline to build from; a rough continuation of the Light→Medium progression (Spd −1, Eva −1, Arm +1, Tuf +3 per step, Mettle flat) was floated as a starting point but never locked, and a steeper Armor/Toughness jump than a strict linear read is probably right for what's meant to be the heaviest thing in the game.
