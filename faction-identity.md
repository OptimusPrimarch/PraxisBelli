...
**Heavies — split into two sub-families, identity locked, individual package menus not yet written.** "Heavies" was never one unit; it splits into a **Terminator family** and a **Gravis family**, both sharing the Arm7/Eva3/Tuf3/`Indomitable` stat line but differentiated by very different survivability and role textures — real lore treats Terminator Armor as fictionally the toughest personal armor in the setting, and this split honors that without needing a heavier stat line to do it.
...
- **Terminator family** — shares Gravis's exact stat line, but its actual differentiator is `Shrug (X)`, not a stat bump. Terminator Armor itself carries a baseline `Shrug` value; equipping the Storm-Shield-equivalent wargear improves it further (lower X = better, confirmed). `Shrug` is Terminator-exclusive — Gravis units never get it. Also carries **`Warded Plate`** (name provisional, may change): *"while this model would take doubled damage from a hit with the Anti-Infantry trait, it instead takes that damage normally."* Direct payoff of a line already sitting in Ablative Plating's own definition — *"industrial materials science, not warded plate... since Regiments has no oath-access at all"* — Oathkeepers, with Deep Oath, are exactly the faction that line was leaving room for. Anti-Infantry weapons still work normally against Gravis; only Terminators get this answer.
  - **Assault Terminators** — SHOCK. Melee-focused, Storm Shield + heavy melee weapon energy, the natural home for the Storm-Shield-equivalent `Shrug` carrier. Carries `Impact (1)`.
  - **Terminator Squad** (ranged) — LINE, despite being far more elite/expensive than a typical LINE unit — deliberately not given a new keyword to represent that; the cost and stat line already carry it, matching the project's standing cost philosophy (stat divergence is cheap, new keywords are the expensive lever). Storm Bolter-equivalent + heavy-weapon swap (Assault Cannon-style automatic, Cyclone Missile Launcher-style indirect).
- **Gravis family** — shares the same stat line, no `Shrug`, no `Warded Plate`; Anti-Infantry works normally against them.
  - **Aggressors** — SHOCK. Integrated twin close-range weapon system, slow, walks forward and unloads at point-blank rather than splitting gun and melee.
  - **Inceptors** — SHOCK, standalone. Ranged-focused, no strong melee — the drop-assault identity earns SHOCK through tempo/disruption, not melee prowess. Baseline carries `Jump Jets` (grants `Flying` plus `Impact (X)` on a successful charge), at **`Impact (3)`** — the highest Impact value in the family, since X tracks the violence of the individual model's impact (an orbital-height drop) more than raw armor weight, not a strict "heavier armor always wins" rule.
  - **Gravis Intercessors** — LINE by default, Heavy HAMR Rifle. Weapon swap to HAMR MG or a Multi-melta-equivalent (ties into `Meltdown`) shifts the unit to SUPPORT — a deliberate, explicit divergence from the vehicle rule that packages shouldn't change CATEGORY (the Marten lesson); infantry is allowed to work differently here, on purpose.
  - **Gravis Devastators** — SUPPORT, matching Regiments' own Heavy Weapons Team. Still needs its own distinct reason to exist alongside Gravis Intercessors' SUPPORT-swap configuration (likely scale — fielding several heavy weapons across the squad at once, rather than one configuration choice for the whole unit) — not yet resolved. Exact weapon options (Multi-melta, Heavy Bolter, possibly more per recent source-material updates) deliberately left for the user's own research rather than my uncertain recollection.
- **Tacticus Assault Squad** — new unit, standard-tier armor (same class as Oathkeeper Squad, not Gravis or Terminator), SHOCK, melee-focused. Optional `Jump Jets` upgrade at **`Impact (1)`** — Jump Jets' second carrier alongside Inceptors, giving the mechanic more than one home.
...
**Open before any of this is locked:** every exact number beyond what's stated above (HAMR Rifle's and HAMR Carbine's own base profiles, all three melee-swap weapons' AP/Damage, `Crushing`'s and the Gravity Pistol's full profile, Oathreapers' grenade name and mechanism, `Deployment (Infiltrate)`'s exact distance, `Optics`'s pricing tier, `Crushing Impact`'s stubbed AP3/D2, `Warded Plate`'s final name); none of the six Heavies-family units (Assault Terminators, Terminator Squad, Aggressors, Inceptors, Gravis Intercessors, Gravis Devastators) or Tacticus Assault Squad have a full package-menu writeup yet — only identity, category, and signature mechanics are locked; Oathmaster has a name and nothing else, deliberately deferred; and the roster still needs whatever units answer the remaining gaps in SUPPORT and ARMOR for this faction.
