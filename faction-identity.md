...
- **Special-weapon swap — Sniper pattern**: the actual signature Scout option. Flagged as the natural home for finally defining `Optics` (currently a priced-at-0 stub — "helps land hits," mechanism never written) — not resolved yet, just the obvious place to resolve it.
...
**Open before any of this is locked:** every exact number beyond what's stated above (HAMR Rifle's and HAMR Carbine's own base profiles, all three melee-swap weapons' AP/Damage, `Crushing`'s and the Gravity Pistol's full profile, Oathreapers' grenade name and mechanism, `Deployment (Infiltrate)`'s exact distance, `Optics`'s actual mechanism); Oathwarden and Heavies/Terminators still have no package writeup at all (Errant now does, above); Oathmaster has a name and nothing else; the Heavies/Terminators-equivalent unit has no name locked yet either; and the roster still needs whatever units answer SUPPORT and ARMOR for this faction beyond the LINE/COMMAND/RECON/SHOCK work done here.
