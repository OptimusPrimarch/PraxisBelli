...
**Full Oathkeepers roster as of this naming pass:** COMMAND — Oathmaster (deferred). LINE — Oathkeeper Squad, Oathwarden Squad, Oathkeeper Paragons. SUPPORT — Oathguard. RECON — Errant Squad. SHOCK — Oathreapers, Oathkeeper Paladins, Oathcrusher Squad, Oathkeeper Assault Squad. ARMOR — still empty; the one real coverage gap left to address.

**Open before any of this is locked:** every exact number beyond what's stated above (HAMR Rifle's and HAMR Carbine's own base profiles, all three melee-swap weapons' AP/Damage, `Crushing`'s and the Gravity Pistol's full profile, Oathreapers' grenade name and mechanism, `Deployment (Infiltrate)`'s exact distance, `Optics`'s pricing tier, `Crushing Impact`'s stubbed AP3/D2, `Warded Plate`'s final name); none of the five now-named Heavies/standard-tier units (Oathkeeper Paladins, Oathkeeper Paragons, Oathcrusher Squad, Oathguard, Oathkeeper Assault Squad) have a full package-menu writeup yet — only identity, category, and signature mechanics are locked; Oathmaster has a name and nothing else, deliberately deferred; and the roster still needs whatever fills the ARMOR gap for this faction.
